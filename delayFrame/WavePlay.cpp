...
#include "..\scrsndcpy\Utility\CodeConvert.h"
#include "..\scrsndcpy\Utility\Logger.h"
...
bool WavePlay::Init(int bufferMultiple, int maxBufferSampleCount)
{
	...
	ATLASSERT(0 <= maxBufferSampleCount && maxBufferSampleCount <= 48000);
	...
	while (!m_exit) {
		...
		{	// Render バッファに書き込む
...

			{
				...
				const int kForceBufferClearSampleCount = m_maxBufferSampleCount * 3;
...
#endif
			::PostMessage(m_hWndMainDlg, WM_WAVEPlAY_INFO, MAKELONG(playSample, dropSample), MAKELONG(minSample, maxSample));
